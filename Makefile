...
HTTPS_GIT := https://github.com/noria-net/ibc-hooks.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=wasm \
		  ...
		  -X github.com/noria-net/ibc-hooks/app.Bech32Prefix=chain \
...
format: format-tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/noria-net/ibc-hooks
